...
def make_cc(dfs=True, uf=True, download=False):
    ...
    if to_download:
        if not download:
            print('there are missing RFCs that are not being downloaded. Run the script with download=True to download')
        else:
            download_rfcs(to_download)

    # DFS based grouping

    # test graph = {'a': ['b'], 'd' : ['r'], 'c' : ['r', 'g'], 'k' : ['n', 'p'], 'e' : ['f'], 'f' : ['l'], 'l' : ['m']}
    # masterset = []
    # seen = set()
